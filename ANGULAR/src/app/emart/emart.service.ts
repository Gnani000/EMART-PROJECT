...
export class EmartService {
  ...
  getAllItems():any{
    return this.http.get('http://localhost:8099/emart/item/all');
  }
  ...
  getItem(itemId: string):any{
    ...
    return this.http.get('http://localhost:8099/emart/item/'+itemId);
  }
  ...
  addBill( todayDate: Date, total: number){
    ...
    this.cartItems = [];
    allBillDetails = [];
    return this.http.post("http://localhost:8099/emart/addBill", bill);
  }
  ...
  validateBuyer(user: string, password: string){
    ...
    return this.http.get('http://localhost:8099/emart/validate', {headers});
  }
